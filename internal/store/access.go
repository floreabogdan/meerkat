...
// ParseAccessWhitelist reads the access-whitelist textarea into prefixes.
func ParseAccessWhitelist(text string) ([]netip.Prefix, []string) {
	return ParsePrefixList(text)
}

// ParsePrefixList reads a prefix textarea — one IP or CIDR per line or
// comma-separated, blank lines and # comments ignored — into prefixes. A bare
// address becomes a host prefix (/32 or /128). Returns the prefixes and a list
// of human-readable errors.
//
// Two settings use this shape: who may reach meerkat, and which networks are
// ours (and so never published to the public threat map).
func ParsePrefixList(text string) ([]netip.Prefix, []string) {
...
// meerkat — i.e. it is non-empty and does not contain a default route. It is the
...
// AccessAllowed reports whether ip may reach meerkat given the whitelist. It
