...
// alerts.go stores alert destinations — the places meerkat delivers a
// notification when something worth waking up for happens. Adapted from the
// sister projects nftably and birdy, which copy it from each other.
//
// The kinds below are deliberately source-level, not event-level. Notifying per
// alert is what flooded a Discord channel in minutes and is the mistake this
// project exists to correct: the unit of interest is a source crossing a
// threshold, not a reputation rule restating itself for the 600th time.
...
// Alert event kinds — what meerkat can notify on. Phase 3 wires these up; they
// are defined here so the destination form has a stable set to subscribe to.
const (
	// A source crossed the interest threshold (N events across M signatures).
	AlertSourceThreshold = "source.threshold"
	// A high-severity signature fired from a source for the first time — the
	// 0.3% of the flood that is an actual compromise indicator.
	AlertSourceSevere = "source.severe"
	// A block was pushed to nftably and took effect.
	AlertSourceBlocked = "source.blocked"
	// eve.json stopped producing, or became unreadable. Silence from a sensor is
	// indistinguishable from peace and quiet unless something says so.
	AlertIngestDown = "ingest.down"
	AlertIngestUp   = "ingest.up"
	// The block API is unreachable, so nothing can be blocked right now.
	AlertNftablyDown = "nftably.down"
...
func AlertEventKinds() []AlertEventKind {
	return []AlertEventKind{
		{AlertSourceSevere, "High-severity signature from a new source"},
		{AlertSourceThreshold, "A source crossed the interest threshold"},
		{AlertSourceBlocked, "A source was blocked"},
		{AlertIngestDown, "Ingest stopped (eve.json silent or unreadable)"},
		{AlertIngestUp, "Ingest recovered"},
		{AlertNftablyDown, "nftably unreachable (cannot block)"},
...
// validator, and meerkat should not reject an address its mailserver accepts.
