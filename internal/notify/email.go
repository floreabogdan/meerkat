...
func sendEmail(d store.Destination, a alert) error {
	...
	subject := a.emoji() + " meerkat: " + a.title()
...
func buildMIME(from string, tos []string, subject, textBody, htmlBody string) string {
	boundary := "meerkat-boundary-0f8a1c"
...
<div style="font-size:12px;letter-spacing:0.08em;text-transform:uppercase;color:` + a.hexColor() + `;font-weight:700">meerkat alert</div>
...
<tr><td style="padding:12px 24px;background:#f9fafb;color:#9ca3af;font-size:12px">Sent by meerkat, your nftables manager.</td></tr>
