...
var dummyPasswordHash, _ = bcrypt.GenerateFromPassword([]byte("meerkat-login-timing-equalizer"), bcrypt.DefaultCost)
...
func (s *Server) handleLoginSubmit(w http.ResponseWriter, r *http.Request) {
	...
	if bcrypt.CompareHashAndPassword(hash, []byte(password)) != nil || !ok {
		if s.login.fail(ip) {
			s.notifier.Notify(store.AlertLoginFailed, ip, "Repeated failed logins to meerkat — that source is now locked out.")
		}
		http.Redirect(w, r, "/login?error=1", http.StatusSeeOther)
		return
	}
	...
	_ = s.store.InsertAudit(user.Username, store.AuditLogin, "signed in")
	http.Redirect(w, r, "/", http.StatusSeeOther)
}
...
// HashPassword is used by `meerkat init` and the password-change flow.
