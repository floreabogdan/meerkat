...
// adoptDBOwnership hands a database created by root to the account meerkat
// actually runs as, so "sudo meerkat init" cannot leave a file the service can
// only read.
//
// This is the single sharpest edge in a packaged install: the deb/rpm creates
// /var/lib/meerkat owned by `meerkat` and a systemd unit with User=meerkat, but
// init is a command a human runs, and a human on a router runs it as root.
// SQLite then opens the root-owned file read-only, meerkat starts anyway, and
...
// packages chown it to meerkat), falling back to an `meerkat` account if one
...
// serviceOwner is the uid/gid meerkat's files should belong to: the owner of
// its state directory, or the `meerkat` account. Root owning the directory
